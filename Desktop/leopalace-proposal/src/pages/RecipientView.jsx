...
function poiPinEl(num, fav) {
  const el = document.createElement("div");
  el.style.cursor = "pointer";
  el.innerHTML = `<div style="position:relative;"><div style="width:24px;height:24px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);background:${fav ? "#f59e0b" : "#003087"};border:2px solid #fff;box-shadow:0 1px 3px rgba(0,0,0,.3);"></div><div style="position:absolute;top:2px;left:0;width:24px;text-align:center;font-size:11px;font-weight:800;color:#fff;">${num}</div></div>`;
  return el;
}

function rvTooltipHTML(p, num) {
  return `
    <div style="min-width:160px;">
      <div style="font-weight:700;font-size:12px;">${num}. ${p.name}${p.unavailable ? "（募集対象外）" : ""}</div>
      <div style="font-size:11px;color:#e11d48;font-weight:700;">${(p.rent / 10000).toFixed(1)}万円</div>
      <div style="font-size:10px;color:#64748b;">${p.station}駅 徒歩${p.walkMin}分</div>
    </div>`;
}

// 受取側の地図検索（MapLibre GL JS）。ピン番号はリスト番号と連動、★はお気に入り。
function RvMap({ points, favorites }) {
  ...
  useEffect(() => {
    ...
    const center = points.length
      ? [
          points.reduce((s, p) => s + (p.lng || 139.68), 0) / points.length,
          points.reduce((s, p) => s + (p.lat || 35.77), 0) / points.length,
        ]
      : [139.68, 35.77];
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      markersRef.current = [];
      points.forEach((p, i) => {
        const el = poiPinEl(i + 1, favorites[p.id]);
        el.addEventListener("mouseenter", () => {
          popupRef.current
            .setLngLat([p.lng, p.lat])
            .setHTML(rvTooltipHTML(p, i + 1))
            .addTo(map);
        });
      ...
      {mode === "map" && (
        <div className="bg-white border border-slate-200 rounded-md overflow-hidden">
          <div className="h-[460px]">
            <RvMap points={props.filter((p) => p.lat && p.lng)} favorites={favorites} />
          </div>
